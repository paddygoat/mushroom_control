#!/usr/bin/env python
# cd /home/paddy/Documents/Mushrooms && python3 data_rx_control_room_05.py
...
def animate(i, xs, yhum, ytempCon, ycoTwo, ypmA, ypmB, ypmC, threeDArray, counter, counter2, delta_time, errorCount):
    # Make a call back:
    # ser.write(b'S\n')
    ...
    if len_char_data >2 and len_char_data <50:
        ...
        if headerContainerTest == True:
            if (myStringLabel == "hum") :
                data_y_hum = float(myStringValue)
                print("hum detected !!")
            if (myStringLabel == "temp"):
                data_y_tempCon = float(myStringValue)
                print("temp detected !!")
            if (myStringLabel == "coTw"):
                data_y_coTwo = float(myStringValue)
                print("coTwo detected !!")
            if (myStringLabel == "pmA"):
                data_y_pmA = float(myStringValue)
                print("pmA detected !!")
            if (myStringLabel == "pmB"):
                data_y_pmB = float(myStringValue)
                print("pmB detected !!")
            if (myStringLabel == "pmC"):
                data_y_pmC = float(myStringValue)
                print("pmC detected !!")
            # temp is the last chucnk of data to be received:
            if myStringLabel == "temp":
                yhum.append(data_y_hum)
                ytempCon.append(data_y_tempCon)
                ycoTwo.append(data_y_coTwo)
                ypmA.append(data_y_pmA)
                ypmB.append(data_y_pmB)
                ypmC.append(data_y_pmC)
...
ani = animation.FuncAnimation(fig, animate, fargs=(xs, yhum, ytempCon, ycoTwo, ypmA, ypmB, ypmC, threeDArray, counter, counter2, delta_time, errorCount), interval=50)
plt.show()
